...
{- | A safe, lazy, and "fold/build" fusible unfolding
    (\'a la 'Data.Stalk.unfold') of 'Data.Stalk.Stalk's
    ...
    > {-# INLINE unfoldST1 #-}
    > unfoldST1 :: forall rs s a b.
    >     (rs -> ST s (Either b (a, rs))) ->
    >     (ST s rs) ->
    >     ST s (Stalk b a)
    > unfoldST1 = \ t yrs0 ->
    >     let unfoldSTR = \ rs ->
    >             t rs >>= \case
    >                 Right (a, rs') -> Extend a <$> unfoldSTR rs'
    >                 Left b         -> pure $ Yield b 
    >     in  unfoldSTR =<< yrs0
    ...
    > {-# INLINE unfoldST3 #-}
    > unfoldST3 :: forall (r :: Type -> Type) a b.
    ...
    >             t r >>= \case
    >                 Right (a, r') -> Extend a <$> unfoldSTR r'
    >                 Left b        -> pure $ Yield b
    ...
    > {-# INLINE unfoldST4 #-}
    > unfoldST4 :: forall (r :: Type -> Type) a b.
    >     (forall s. r s -> ST s (Either b (a, r s))) ->
    >     (forall s. ST s (r s)) ->
    >     Stalk b a
    > unfoldST4 = \ t yr0 ->
